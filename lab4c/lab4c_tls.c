...
int main(int argc, char * argv[]){
	...
	while(1){
		...
		if(opt_stop_or_start == 1 && ts.tv_sec >= next_report_time){ 
			char buffer_ssl[256]; //buffer to be used for encripting and storing
			if(sprintf(buffer_ssl,"%02d:%02d:%02d %.1f\n",tm->tm_hour, tm->tm_min, tm->tm_sec, temperature_val) < 0){ //https://stackoverflow.com/questions/19431674/rounding-floats-to-nearest-10th
				fprintf(stderr, "Error: Could not append characters to buffer_ssl\n");
				exit(2);
			} 
			if(SSL_write(ssl_client, buffer_ssl, strlen(buffer_ssl)) <= 0){
				fprintf(stderr, "Error: Could not write the number of bytes from buffer_ssl into the specified ssl connection.\n");
				exit(2);
			}
...
void read_input(){
	...
	int readable;
	if((readable = SSL_read(ssl_client, char_buff, sizeof(char_buff))) <= 0){ //reads from ssl_client and store into char_buf
		fprintf(stderr, "Error: Could not read from specified ssl connection.\n");
		exit(1);
	}
	int k = 0;
	while(k < readable){
...
void options_compare(char compare_buff[]){
	...
	//https://www.tutorialspoint.com/c_standard_library/c_function_strstr.htm
		...
		char buffer_ssl[256];
		if(sprintf(buffer_ssl, "Error: Option provided was not valid. Please try another.\n") < 0){
			fprintf(stderr, "Error: Could not append characters to buffer_ssl.\n");
			exit(2);
		}
		if(SSL_write(ssl_client, buffer_ssl, strlen(buffer_ssl)) <= 0){
...
void turn_off(){
	...
	tm = localtime(&(ts.tv_sec));
	char buffer_ssl[256];
	if(sprintf(buffer_ssl,"%02d:%02d:%02d SHUTDOWN\n", tm->tm_hour, tm->tm_min, tm->tm_sec) < 0){
		fprintf(stderr, "Error: Could not append characters to buffer_ssl.\n");
		exit(2);
	}
	if(SSL_write(ssl_client, buffer_ssl, strlen(buffer_ssl)) <= 0){
...
SSL * attach_ssl_to_socket(){
	...
	if(SSL_set_fd(sslClient, socket_fd) == 0){
		fprintf(stderr, "Error: Could not set socket_fd as the input/output facility for TLS/SSL side of ssl.\n");
		exit(2);
	}
	if(SSL_connect(sslClient) != 1){
		fprintf(stderr, "Error: TLS/SSL handshake was not successful. TLS/SSL connection not established.\n");
		exit(2);
	}
	return sslClient;
}
...
void ssl_clean_client(){
	if(SSL_shutdown(ssl_client) != 1){
		fprintf(stderr, "Error: Shutdown was not successful.\n");
		exit(2);
	}
	SSL_free(ssl_client);
}